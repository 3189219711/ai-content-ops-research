# -*- coding: utf-8 -*-
"""把 数字人唱歌类账号单拆汇总.md 转成单文件 HTML（图片 base64 内嵌）。"""
import os, re, html, base64

SRC = r"D:/TOJOY/AI内容运营开源项目调研/账号单拆/数字人唱歌类账号单拆汇总.md"
BASE = os.path.dirname(SRC)
OUT = r"D:/TOJOY/AI内容运营开源项目调研/账号单拆/数字人唱歌类账号单拆汇总.html"
    ...
    if mh:
        ...
        while i < n:
            ...
            if i < n:
                ...
                if m2:
                    figs.append(embed_image(m2.group(2), m2.group(1)))
                    i += 1
                    continue
            break
        if not figs:
            ...
            media_html = '<div class="multi-fig">' + ''.join(figs) + '</div>'
        cell = f'''<div class="work-cell">
  <div class="work-media">{media_html}</div>
  <div class="work-meta">
    {h4_html}
    {ul_html}
  </div>
...
.work-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:14px;margin:16px 0;align-items:start;}
...
.multi-fig{display:grid;grid-template-columns:1fr 1fr;gap:2px;background:#f0f0f0;}
.multi-fig .shot{margin:0;border:none;border-radius:0;box-shadow:none;}
...
<title>数字人唱歌类账号单拆汇总</title>
<style>""" + CSS + """</style>
</head>
<body>
<div class="hero">
  <h1>数字人唱歌类账号单拆汇总</h1>
  <p>4 个对标账号 · 爆款公式与作品清单 · 复刻对比与侵权风险 · 内部参考</p>
</div>
...
print(f"作品卡片：{body_html.count('work-cell')} 个")
print(f"双图卡片：{body_html.count('multi-fig')} 个")
