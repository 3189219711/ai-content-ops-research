# -*- coding: utf-8 -*-
"""把 AI萌宠类账号单拆汇总.md 转成单文件 HTML（图片 base64 内嵌）。"""
import os, re, html, base64

SRC = r"D:/TOJOY/AI内容运营开源项目调研/账号单拆/AI萌宠类账号单拆汇总.md"
BASE = os.path.dirname(SRC)
OUT = r"D:/TOJOY/AI内容运营开源项目调研/账号单拆/AI萌宠类账号单拆汇总.html"
...
while i < n:
    ...
    if m:
        flush_account_cells()
        out.append(embed_image(m.group(2), m.group(1))); i += 1; continue
  ...
  <div class="work-meta">
    ...
    if re.match(r'^-\s+', line):
        ...
        while i < n and re.match(r'^-\s+', lines[i]):
            items.append('<li>' + md_inline(re.sub(r'^-\s+', '', lines[i])) + '</li>'); i += 1
...
.tag{display:inline-block;background:var(--soft);color:var(--accent);font-size:12px;padding:2px 8px;border-radius:6px;margin-right:6px;}
...
<title>AI 萌宠类账号单拆汇总</title>
<style>""" + CSS + """</style>
</head>
<body>
<div class="hero">
  <h1>AI 萌宠类账号单拆汇总</h1>
  <p>5 个抖音 AI 萌宠账号 · 单拆拆解 + 跨账号复刻对比 · 内部参考</p>
</div>
